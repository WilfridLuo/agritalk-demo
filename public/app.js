...
function renderCaseRunner() {
  ...
  function startRun() {
    ...
    pushMessages([{ role: "user", label: "Farmer input", text: queryInput.value.trim() || activeCase.query }]);
    fillResults({
      confidence: 18,
    ...
    window.setTimeout(() => {
      ...
      const clarifies = activeCase.firstPass.stageLabel === "Clarify";
      renderWorkflow(clarifies ? 2 : 3, clarifies ? 3 : 4);
      updateDecisionPill(clarifies ? "clarify" : "respond", activeCase.firstPass.stageLabel);
